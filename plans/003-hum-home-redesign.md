...
- One new Railway project `froggy-redesign` (owner's account, region `europe-west4`), two services `hum-fable` and `hum-codex`, each a GitHub-connected service on its branch, built from the repo `Dockerfile`. Two variables each: `APP_ORIGIN=http://localhost:3000` and `EXTRA_ORIGINS` set to the service's public URL. The server only accepts WebSocket upgrades from its allowed origins (403 otherwise), and it refuses to boot with stub adapters unless `APP_ORIGIN` is loopback (`refuseStubbedBoot` in `apps/server/src/environment.ts`), which is exactly what `EXTRA_ORIGINS` exists for. Found on the first live deploy on 13 Sep: the page rendered, the socket was refused, and setting `APP_ORIGIN` to the public URL crashed the boot. Everything else stays stub. `PORT` is read by the server; Railway injects it.
